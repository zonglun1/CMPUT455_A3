...
class GoBoard(object):
    ...
    def neighbors_of_color_1(self, point):
    ...
    def neighbors_of_point(self, point):
        nbc = []
        for nb in self._neighbors(point):
            if self.get_color(nb) != EMPTY and self.get_color(nb) != BORDER:
                nbc.append(nb)
        for nb in self._diag_neighbors(point):
            if self.get_color(nb) != EMPTY and self.get_color(nb) != BORDER:
                nbc.append(nb)
    ...
    def detect_five_in_a_row(self):
        ...
        if self.moves == []:
            return EMPTY
        cur_move = self.moves[-1]
        cur_color = self.get_color(cur_move)
        args = [cur_color, format_point(point_to_coord(cur_move, self.size))]

        # check horizontal
        hori_count = self.check(args, [0, 1], 0) + self.check(args, [0, -1], 0)
        if hori_count + 1 >= 5:
            return cur_color
        # check vertical
        ver_count = self.check(args, [1, 0], 0) + self.check(args, [-1, 0], 0)
        if ver_count + 1 >= 5:
            return cur_color
        # check diagonal(left top)
        left_diag_count = self.check(args, [-1, -1], 0) + self.check(args, [1, 1], 0)
        if left_diag_count + 1 >= 5:
            return cur_color
        # check diagonal(right top)
        right_diag_count = self.check(args, [-1, 1], 0) + self.check(args, [1, -1], 0)
        if right_diag_count + 1 >= 5:
            return cur_color
    ...
    def check(self, args, dir, count):
        ...
        coord = move_to_coord(board_move, self.size)
        move = format_point(coord)
        # get next point info
        next_coord = [coord[0] + dir[0], coord[1] + dir[1]]
        next_move = format_point(next_coord)
        row = next_coord[0]
        col = next_coord[1]
        point = self.NS * row + col
        if self.get_color(point) == color:
            next_color = self.get_color(point)
            new_args = [board_color, next_move]
            return self.check(new_args, dir, count + 1)
        else:
            return count
        return
    ...
    def int_to_color(self, color_int):
        int_to_color = {BLACK: "b", WHITE: "w", EMPTY: "e", BORDER: "BORDER"}

        try:
            return int_to_color[color_int]
        except:
            raise KeyError("\"{}\" wrong color".format(color_int))

    def code(self):
        c = 0
        for i in range(self.size**2):
            c = self.size*c + self.board[i]
        return c
...
######## Assignments3 ######
    ...
    def moveNumber(self):
        ...
        -------
        this function will return the number of moves until now
        '''
        return len(self.moves)
    ...
    def simulate(self):
        ...
        Returns the winner and the number of steps
        -------
